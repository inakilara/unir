...
int csvAbinario(char *path){
  Tcliente *lista= NULL;  // Creamos una lista dinámica en la que ir leyendo los registros
  ...
  if (file!=NULL) {  // Comprueba que se ha abierto el archivo
    while(fgets(myString,50, file)) {        //Se detiene si llega al final el archivo
      ...
      Tcliente *temp=(Tcliente*)realloc(lista,i*sizeof(Tcliente));   // Ampliamos la lista en 1 en cada vuelta del bucle
      if (temp==NULL) {
        free(lista);
        free(temp);
        return -1;

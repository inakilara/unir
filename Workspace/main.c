...
void mostrar_registros(gasolinera[],int);
...
int main(){
    FILE *archivo = fopen("../Workspace/gasolineras.csv","r");
    ...
    while(opcion!='q') {
        ...
        switch (opcion) {
            case 'a': {
                mostrar_registros(gasolineras,len);
                break;
            }
            case 'b': {
...
void mostrar_registros(gasolinera gasolineras[],int len) {
    for(int i=0;i<len;i++) {
